  ...
  return (
    <footer className="px-3 pb-6 sm:px-4 lg:pl-2 lg:pr-4">
      <div className="px-5 pt-5 sm:px-7 lg:px-9">
        <div className="flex flex-col gap-7 pb-5 xl:flex-row xl:justify-between xl:gap-10">
          ...
          <nav aria-label="Footer" className="grid grid-cols-3 gap-4 sm:gap-10">
            {linkGroups.map((group) => (
              <div key={group.label}>
                <h2 className="mb-1 text-[11px] font-semibold uppercase tracking-wider text-muted">
                  {group.label}
                </h2>
                <ul>
                  {group.links.map((link) => (
                    <li key={link.href}>
                      <a
                        href={link.href}
                        className="inline-flex min-h-11 items-center rounded-sm py-2 text-[13px] leading-5 text-muted underline-offset-4 transition-colors hover:text-foreground hover:underline focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-accent"
                      >
                        {link.label}
                      </a>
                    </li>
        ...
        <div className="border-t border-border-strong pt-5 text-[12px] leading-6 text-muted">
          <p>
